...
void setup() {
  ...
  for(int i = 0; i < 2; ++i){
    ...
    Serial2 << "w axis" << i << ".controller.config.vel_limit" << 22000.0f << '\n';
    Serial2 << "w axis" << i << ".controller.config.current_lim" << 11.0f << '\n';
  }
...
void loop() {
  ...
  if(character == '0' || character == 'l'){
    ...
    if(!odrive2.run_state(num, requested_state, true)) return;

    requested_state = ODriveArduino::AXIS_STATE_ENCODER_OFFSET_CALIBRATION;
    Serial << "Axis" << character << ": Requesting state " << requested_state << '\n';
    if(!odrive1.run_state(num, requested_state, true, 25.0f)) return;
    if(!odrive2.run_state(num, requested_state, true, 25.0f)) return;

    requested_state = ODriveArduino::AXIS_STATE_CLOSED_LOOP_CONTROL;
    Serial << "Axis" << character << ": Requesting state " << requested_state << '\n';
    if(!odrive1.run_state(num, requested_state, false /*don't wait*/)) return;
    if(!odrive2.run_state(num, requested_state, false /*don't wait*/)) return;
  }
  if(character == 'b'){
    odrive1_serial  << "r vbus_voltage\n";
    odrive2_serial  << "r vbus_voltage\n";
    Serial << "VBus Voltage for ODrive1: " << odrive1.readFloat() << '\n';
    Serial << "VBus Voltage for ODrive2: " << odrive1.readFloat() << '\n';
  }
  if(character == 'p'){
    ...
    Serial2 << "w axis" << 0 << ".controller.config.vel_limit " << 200000.0f << '\n';
